...
**已验证（Task 3 实现时，2026-07-30）**：并排截图对比线上旧版
`https://abi-breast-biomechanics-group.github.io/breast-educational-resource/density-3`
与 `/cancer-dcis`，同本仓库 `rebuild/foundation` 分支的 `/density-c/mri`、
`/cancer-dcis/mri`。结果：旧版本身就是 density 组「胸壁居上、双侧乳房垂向
下方」、lesion 组「乳头区居上、组织向下收窄的单侧切面」这两种互不相同的取
景，且与本仓库当前渲染逐一对应、方向一致（不是本仓库引入的新漂移）。也就
是说 density/lesion 两组的 up 向量差异是旧版预设 JSON 自带的既有状态，
`fitToView.ts` 只改了距离、未改方向，也没有让这个差异变得更明显或更隐蔽。
按上一段的既定处理：本任务未改动任何 `upVector`，问题原样留给甲方确认，
不在本轮统一。

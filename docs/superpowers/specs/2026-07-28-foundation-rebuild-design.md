...
| ~~停留 >1500ms 后预取下一模态~~ | **已实测否决，见下方修正** |
| `the-breast` 首屏仅加载 `density25.glb` | Draco 后约 2MB，零 NRRD |

**人均实际下载量预计从 ~450MB 降至 20–40MB**（少有用户遍历全部 10 个病例 × 全部模态）。

> **实测修正（2026-07-30）：预取一行删除，不实现。**
>
> 预取的前提是「`fetch()` 暖了 HTTP 缓存，真正的加载就能命中」。该前提对本
> 目录的资产**不成立**——不是因为 `fetch` 与 copper3d 的 XHR 不共享缓存
> （它们共享），而是因为**响应体太大，Chromium 根本不存**。单条缓存条目
> 上限是磁盘缓存总容量的一个分数，实测在本机 Playwright 默认 profile 下落
> 在 6–8MB 之间。
>
> 实测方法与数字（`.output/public` 由 `nuxi generate` 产出，以 GitHub Pages
> 自己的 `Cache-Control: max-age=600` + ETag 服务，用 CDP 的
> `Network.loadingFinished.encodedDataLength` 计真实上行字节）：
>
> | 资产 | 暖机字节 | 真实加载字节 | 缓存命中 |
> |---|---|---|---|
> | `density25.glb` 0.6MB | 659,858 | 0 | ✅ disk |
> | `density-4/middle/m3d.nrrd` 10.5MB | 10,972,779 | 10,972,779 | ❌ 无 |
>
> 合成文件二分：2/4/6MB 命中，8/10/12/16MB 全部落空。
>
> 而预取**只会**指向非首个模态。全目录 16 个这样的资产里，15 个在
> 7.7–50.8MB，唯一低于上限的是 `benign-cyst/middle/u2d.nrrd`（50KB 的超声
> 占位）。解剖 GLB 永远排在序列第一位，因此永远不会成为预取目标。
>
> 也就是说：这个预取在本 app 里**结构上不可能生效**，而每次触发都让读者为
> 同一个体数据付两次流量。在一个可能用手机流量打开的教学 app 里，这比没有
> 预取更糟。**删除**。
>
> 若日后确实要做，可行路径是 Service Worker + Cache API：`caches.put()` 不
> 受上述单条目上限约束（只受 origin 存储配额约束），且 SW 的 `fetch` 事件
> 能同时服务 `fetch` 与 XHR。代价是静态托管上多一个 SW 生命周期与缓存失效
> 问题，超出本 spec 范围。

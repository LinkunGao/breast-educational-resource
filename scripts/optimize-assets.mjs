 ...
 *
 * Idempotent, so it is safe to re-run. An NRRD already declaring
 * encoding: gzip is not recompressed (see gzipNrrd in nrrd-gzip.mjs); GLB
 * and JSON are regenerated or recopied every time, since Draco compression
 * and a plain copy are both idempotent in themselves — identical input
 * yields identical or equivalent output — so no bookkeeping is needed.
 *
